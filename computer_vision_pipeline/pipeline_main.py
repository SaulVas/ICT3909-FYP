...
def evaluate_pipeline():
    ...
    # --- Create Plot: Average L per Spline with Global Std Dev Lines ---
    ...
    # Store handles for a single legend
    legend_handles = []
    added_global_lines_legend = False
    ...
    for i, spline_idx in enumerate(["0", "1", "2"]):
        ...
        y_values = [plot_data["avg"][spline_idx][idx] for idx in valid_indices]
        y_errors = [plot_data["std"][spline_idx][idx] for idx in valid_indices]
        y_errors = [err if not np.isnan(err) else 0 for err in y_errors]
        subdir_labels_for_plot = [
            plot_data["subdir_keys"][idx] for idx in valid_indices
        ]
        x_coords = range(len(valid_indices))

        # Plot per-subdirectory averages with their std dev error bars
        if valid_indices:
            ebar = ax.errorbar(
                x_coords,
                y_values,
                ...
                capsize=3,
                alpha=0.7,
                label=f"Subdir Avg L \u00b1 Subdir Std Dev" if i == 0 else "",
            )
            if i == 0:  # Add handle only once
            ...
            ax.set_xticklabels(subdir_labels_for_plot, rotation=90, fontsize=8)
            ...
            if not np.isnan(global_avg):
                line_avg = ax.axhline(
                    ...
                    linestyle="--",
                    linewidth=2,
                    label="Global Avg L" if not added_global_lines_legend else "",
                )
                if not added_global_lines_legend:
                ...
                if not np.isnan(global_std):
                    ...
                    ax.axhline(
                        ...
                        linestyle=":",
                        linewidth=2,
                    )  # No label for minus line
                    if not added_global_lines_legend:
                        legend_handles.append(line_std_plus)
                        added_global_lines_legend = True  # Mark as added
        else:
    ...
    # Save Figure
    plot_filename = "average_l_per_spline_with_global_stats.png"  # Updated filename
    try:
        plt.savefig(plot_filename, bbox_inches="tight")
        print(f"\nSaved plot to: {plot_filename}")
    except Exception as e:
        print(f"Error saving plot: {e}")
    plt.close(fig_avg)  # Close the figure
